...
// Every admin write from the match-rules page goes through here: admin + CSRF, as
// those endpoints demand. Not only match rules — the manufacturer-alias endpoints
// on the same page want exactly the same envelope.
async function sendAdminWrite(url, method, payload) {
  return fetch(url, {
    method,
